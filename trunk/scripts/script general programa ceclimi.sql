...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertarCirugia`(IN `NOMBRE` varchar(15),IN `DESCRIPCION` varchar(15))
    DETERMINISTIC
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertarCirugiaPaqueteF`(IN  `PROTESIS` float, IN `FECHA_OPERACION` date,IN `DESCUENTO` FLOAT(12,2),IN `MONTO_CIRUJANO` FLOAT(12,2),IN `CIRUGIA` bigint,IN `CIRUJANO` bigint,IN `PAQUETE` bigint, OUT `MAX_ID` bigint)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertarPaciente`(IN `CEDULA` bigint,IN `PRIMERNOMBRE` varchar(20),IN `SEGUNDONOMBRE` varchar(20),IN `PRIMERAPELLIDO` varchar(20),IN `SEGUNDOAPELLIDO` varchar(20),IN `TELEFONOFIJO` varchar(10),IN `TELEFONOMOVIL` varchar(10),IN `CORREO` varchar(20),IN `FECHA_INGRESO` date)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertarPago`(IN `IDPAGO` bigint,IN `MONTO` float(12,2),IN `NOMBRE` varchar(45),IN `FECHA` DATE,IN `SEGURO` VARCHAR(30), IN `TIPO_PAGO` VARCHAR(3),IN `PACIENTE` varchar(45))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertarPaqueteFinanciero`(IN `FECHA_PAQUETE` date,IN `PACIENTE` bigint,OUT `idMax` bigint)
    DETERMINISTIC
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ValidarPagoExistente`(IN `IDPAGO` bigint,OUT `RESULTADO` bigint)
BEGIN
